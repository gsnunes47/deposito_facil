...
export async function createProduto(request: Request, response: Response) {
    ...
    if (!request.body.nome || request.body.nome.trim() === "") {
        return response.status(400).send({ error: "Nome do produto é obrigatório" });
    }
...
export async function deleteProduto(request: Request, response: Response) {
    ...
    if (!produtoId) {
        ...
        return response.status(400).send({ error: "ID do produto deve ser um número" })
    }
...
export async function updateProduto(request: Request, response: Response) {
    ...
    if (request.body.quantidade == null || request.body.quantidade === "" || isNaN(Number(request.body.quantidade))) {
        return response.status(400).send({ error: "Quantidade do produto é obrigatória e deve ser um número" });
    }
