...
export async function createCliente(request: Request, response: Response) {
    ...
    if (!request.body.nome || request.body.nome.trim() === "") {
        return response.status(400).send({ error: "Nome do cliente é obrigatório" });
    }

    const cliente = await clienteDomain.createCliente(request.body.nome, user.tenantId, request.body.documento ?? "")

    if (cliente.code === 200) {
        return response.status(200).send({ message: cliente.message, cliente_id: cliente.cliente_id })
    } else {
        return response.status(400).send({ message: cliente.message, error: cliente.error })
    }
...
export async function deleteCliente(request: Request, response: Response) {
    ...
    const cliente = await clienteDomain.deleteCliente(Number(clienteId), request.user.tenantId)
